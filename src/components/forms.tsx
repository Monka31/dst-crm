...
export function InteractionModal({
  ...
  /**
   * Un message reçu fait sortir l'entreprise de l'attente, mais vers quoi
   * dépend de ce que dit la réponse : c'est donc le résultat choisi qui
   * détermine le statut proposé, et non le seul fait d'avoir reçu un message.
   * La proposition disparaît dès que l'utilisateur touche au sélecteur.
   */
  const statutSuggere = (sens: string, resultat: string) => {
    if (sens !== "recu") return "";
  ...
  const choisirSens = (sens: string) => {
    ...
    if (!statutTouche) setNewStatus(statutSuggere(sens, f.outcome ?? "en_attente"));
  };

  const choisirResultat = (resultat: string) => {
    set("outcome", resultat);
    if (!statutTouche) setNewStatus(statutSuggere(f.direction ?? "envoye", resultat));
  };
