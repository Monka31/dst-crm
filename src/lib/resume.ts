...
const STATUTS: Record<string, string> = {
  ...
  relance_faite: "relance faite, en attente",
...
export function phrase(evenements: Evenement[]): string {
  ...
  // Les messages reçus se racontent dans l'autre sens. Les compter avec les
  // envois donnerait « envoyé 1 email » là où c'est l'entreprise qui a écrit.
  const recus = par("interaction_received");
  if (recus.length) {
    const de = enumerer(recus.map(nomEntreprise));
    segments.push(`reçu ${pluriel(recus.length, "réponse")}${de ? ` de ${de}` : ""}`);
  }
...
export function faitsMarquants(evenements: Evenement[]) {
  ...
  return {
    // Un passage en discussion est une bonne nouvelle au même titre qu'une
    // réponse positive : les deux méritent d'apparaître dans le résumé du jour.
    positifs: [...vers("positif"), ...vers("en_discussion")],
    visites: vers("visite_confirmee"),
    refus: [...vers("refus"), ...vers("abandonne")],
    // Une entreprise contactée passe maintenant en « attente de réponse ».
    contactees: [...vers("en_attente"), ...vers("contact_en_cours")],
  };
}
