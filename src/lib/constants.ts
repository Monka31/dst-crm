export type CompanyStatus =
  | "a_identifier" | "a_contacter" | "contact_en_cours" | "en_attente" // « contact_en_cours » n'est plus proposé, conservé pour le journal
  | "relance" | "relance_faite" | "positif" | "en_discussion" | "visite_confirmee" | "refus" | "abandonne";
...
export const STATUSES: {
  ...
  { key: "relance_faite", label: "Relance faite, en attente", dot: "bg-yellow-500",
    chip: "bg-yellow-100 text-yellow-800 dark:bg-yellow-950 dark:text-yellow-300", kanban: "border-t-yellow-500" },
  ...
  // L'entreprise a dit oui sur le principe et l'échange est engagé, mais rien
  // n'est calé. Sans cet état, ces dossiers restaient en « réponse positive »
  // et on ne voyait plus lesquels demandaient un suivi actif.
  { key: "en_discussion", label: "En discussion", dot: "bg-teal-500",
    chip: "bg-teal-100 text-teal-800 dark:bg-teal-950 dark:text-teal-300", kanban: "border-t-teal-500" },
...
/**
 * Sens d'un message dans un fil.
 *
 * Sans cette distinction, un fil mélange nos envois et les réponses de
 * l'entreprise sans qu'on sache qui a écrit quoi.
 */
export const DIRECTIONS = [
  { key: "envoye", label: "Envoyé", court: "Nous",
    chip: "bg-brand-100 text-brand-700 dark:bg-brand-900/40 dark:text-brand-300",
    puce: "bg-brand-500" },
  { key: "recu", label: "Reçu", court: "Eux",
    chip: "bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300",
    puce: "bg-emerald-500" },
];

export const directionMeta = (d?: string | null) =>
  DIRECTIONS.find((x) => x.key === d) ?? DIRECTIONS[0];
