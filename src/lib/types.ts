...
export type Company = {
  ...
  /** Nombre d'interactions, renvoyé par l'agrégat de PostgREST. */
...
export type Contact = {
  ...
  company?: {
    ...
    // Repris dans les modèles de messages : {{ville}}, {{secteur}} et
    // {{date_dernier_contact}} viennent de l'entreprise, pas du contact.
    city?: string | null; sector?: string | null; last_interaction_at?: string | null;
  } | null;
};
...
export type Interaction = {
  id: string;
  company_id: string;
  /** Premier interlocuteur — conservé pour les écrans qui s'appuient dessus. */
  ...
  /** Message d'origine quand cette interaction est une réponse. */
  parent_id: string | null;
  /** « envoye » pour nos messages, « recu » pour ceux de l'entreprise. */
  direction: string;
  /** Horodatage de la saisie : départage deux messages du même jour. */
  created_at?: string | null;
  author?: Pick<Profile, "first_name" | "last_name"> | null;
  contact?: Pick<Contact, "first_name" | "last_name"> | null;
  company?: { name: string } | null;
  /** Tous les interlocuteurs du message, via la table de liaison. */
  interaction_contacts?: { contact: Pick<Contact, "id" | "first_name" | "last_name"> | null }[];
  /** Réponses rattachées, reconstituées côté client. */
  reponses?: Interaction[];
};
...
export type Settings = {
  ...
  /** L'onglet Statistiques est masqué par défaut. */
