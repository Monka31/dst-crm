...
export default function SettingsPage() {
  ...
  const save = async () => {
    ...
    const { error } = await supabase.from("settings").update({
      ...
      show_statistics: !!f.show_statistics,
    }).eq("id", true);
    setBusy(false);
    ...
    <div className="max-w-3xl space-y-5">
      ...
      <Card className="p-5">
        ...
        <label className="mt-2 flex items-start gap-2 text-sm text-slate-600 dark:text-slate-300">
          <input type="checkbox" checked={!!f.show_statistics}
            onChange={(e) => set("show_statistics", e.target.checked)}
            className="mt-0.5 h-4 w-4 accent-brand-500" />
          <span>
            Afficher l&apos;onglet « Statistiques »
            <span className="mt-0.5 block text-[11px] leading-snug text-slate-400">
              Masqué par défaut : les canaux d&apos;activité et la répartition par secteur
              sont déjà sur le tableau de bord.
            </span>
          </span>
        </label>
        <label className="mt-2 flex items-start gap-2 text-sm text-slate-600 dark:text-slate-300">
