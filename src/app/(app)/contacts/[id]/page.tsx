...
import { ArrowLeft, Mail, MapPin, Phone, Linkedin, Pencil, Star, Plus, Send } from "lucide-react";
...
export default function FicheContact() {
  ...
  const [ecrireOpen, setEcrireOpen] = useState(false);
  /** Liste d'origine, filtres compris. Lue après le montage pour ne pas
   *  faire diverger le rendu serveur du rendu client. */
  const [retour, setRetour] = useState("/contacts");
  useEffect(() => { setRetour(lienRetourListe("contacts")); }, []);
  ...
  const load = useCallback(async () => {
    ...
    const [c, n, i, co] = await Promise.all([
      supabase.from("contacts")
        .select("*, company:companies(id,name,status,logo_url,website_domain,city,sector,last_interaction_at), referrer:profiles!contacts_referred_by_fkey(id,first_name,last_name)")
  ...
  return (
    <div className="space-y-4">
      <Link href={retour} className="inline-flex items-center gap-1.5 text-[12.5px] text-slate-500 hover:text-brand-700">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          {canWrite && (
            <div className="flex gap-2">
              {/* Les modèles s'ouvrent par-dessus la fiche : on copie, on
                  ferme, et on est encore là pour enregistrer l'interaction.
                  Y aller par une autre page obligeait à faire l'aller-retour
                  et à retrouver le contact dans une liste de 400 entrées. */}
              <Button size="sm" onClick={() => setEcrireOpen(true)}>
                <Send size={13} /> Écrire
              </Button>
      ...
      <MessageModal open={ecrireOpen} onClose={() => setEcrireOpen(false)}
        contact={contact}
        entreprise={contact.company
          ? { id: contact.company.id, name: contact.company.name,
              city: contact.company.city ?? null, sector: contact.company.sector ?? null,
              last_interaction_at: contact.company.last_interaction_at ?? null }
          : null} />
