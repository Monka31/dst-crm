...
export default function CompaniesPage() {
  const { poles, canWrite, profile, isAdmin, editeurs } = useApp();
   ...
   *
   * Sans cela, ouvrir une entreprise puis revenir en arrière ramenait la liste
   * complète : le filtre « mes entreprises », la recherche et la page en cours
   * étaient perdus à chaque aller-retour. En les écrivant dans l'URL, le
   * bouton retour du navigateur les restitue tout seul, et un lien vers une
   * liste filtrée devient partageable.
   *
   * On écrit avec replaceState plutôt qu'avec le routeur : cela évite
   * d'empiler une entrée d'historique à chaque frappe dans la recherche.
   */
  ...
  useEffect(() => {
    ...
    // Le lien « mes entreprises » du tableau de bord a besoin du profil.
    if (p.get("mine") && !profile) return;

    if (p.get("mine") && profile) setOwner(profile.id);
    else if (p.get("owner")) setOwner(p.get("owner")!);
    if (p.get("status")) setStatus(p.get("status")!);
    if (p.get("pole")) setPole(p.get("pole")!);
    if (p.get("priorite")) setPriority(p.get("priorite")!);
    if (p.get("q")) setQ(p.get("q")!);
    if (p.get("tri")) {
      const [col, sens] = p.get("tri")!.split(".");
      if (col) setSort({ col, asc: sens === "asc" });
    }
    const n = Number(p.get("page"));
    if (Number.isFinite(n) && n > 1) setPage(n - 1);
  ...
  useEffect(() => {
    ...
    if (pole) p.set("pole", pole);
    if (owner) p.set("owner", owner);
    if (priority) p.set("priorite", priority);
    if (sort.col !== "created_at" || sort.asc) p.set("tri", `${sort.col}.${sort.asc ? "asc" : "desc"}`);
    ...
    // La flèche de la fiche entreprise est un lien vers la liste, pas un
    // retour en arrière : sans cette mémorisation elle ramènerait la liste
    // complète, filtres perdus.
    memoriserListe("entreprises", adresse);
  }, [pret, q, status, pole, owner, priority, sort, page]);
  ...
  const load = useCallback(async () => {
    const { data } = await supabase
      ...
      .select(
        ...
        "pole:poles(id,name,color), owner:profiles!companies_owner_id_fkey(id,first_name,last_name), contacts(count), interactions(count)"
      )
  ...
  useEffect(() => { load(); }, [load]);
  useRealtime(["companies", "contacts", "interactions"], load, "companies");
  ...
  // Changer un filtre renvoie en page 1, mais pas la restauration initiale,
  // qui vient justement de replacer l'utilisateur sur sa page.
  const premierPassage = useRef(true);
  useEffect(() => {
    if (!pret) return;
    if (premierPassage.current) { premierPassage.current = false; return; }
    setPage(0);
  }, [pret, q, status, pole, owner, priority]);
  ...
  const exportCsv = async () => {
    const { data } = await supabase
      .from("companies")
      .select("*, pole:poles(name), owner:profiles!companies_owner_id_fkey(first_name,last_name), contacts(count), interactions(count)")
    ...
    download("entreprises-dst.csv", toCsv(complet.map((r) => ({
      ...
      Interactions: r.interactions?.[0]?.count ?? 0,
            ...
            <Card key={r.id} className="flex flex-col overflow-hidden transition-shadow hover:shadow-md">
              ...
              <div className="flex flex-1 flex-col gap-2 p-3.5">
                ...
                <p className="text-[11px] text-slate-400">
                  ...
                  {r.contacts?.[0]?.count ?? 0} contact(s) · {r.interactions?.[0]?.count ?? 0} interaction(s)
                </p>
              </div>
            </Card>
          ))}
                ...
                {pageRows.map((r) => (
                  <tr key={r.id} className="row-hover">
                    ...
                    <td className="td text-center">
                      {/* Une entreprise sans interaction n'a jamais été travaillée :
                          le zéro doit se voir, pas se fondre dans la colonne. */}
                      {r.interactions?.[0]?.count
                        ? <span className="font-medium text-slate-700 dark:text-slate-200">{r.interactions[0].count}</span>
