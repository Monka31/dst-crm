...
import {
  ArrowLeft, BellPlus, CalendarPlus, Globe, Linkedin, Mail, MapPin, MessageSquare, MessageSquarePlus, Pencil, Phone, Plus, Send, Star, Trash2,
} from "lucide-react";
...
import { lienRetourListe } from "@/lib/retour";
import { CompanyLogo } from "@/components/CompanyLogo";
import { ModaleNotesContact } from "@/components/NotesContact";
import { CHANNELS, OUTCOMES, PRIORITIES, RELATION_TYPES, SOURCES, STATUSES, directionMeta, labelOf, statusMeta } from "@/lib/constants";
import { fmtDate, fmtDateTime, fullName, initials, relative } from "@/lib/format";
import { enFils } from "@/lib/fils";
...
export default function CompanyPage() {
  ...
  const { canWrite, isAdmin, profile, editeurs } = useApp();
  ...
  /** Contact à qui écrire : ouvre les modèles par-dessus la fiche. */
  const [ecrireA, setEcrireA] = useState<Contact | null>(null);
  /** Adresse de la liste telle qu'elle était, filtres compris. Lue après le
   *  montage : lire le stockage pendant le rendu ferait diverger le HTML
   *  du serveur de celui du client. */
  const [retour, setRetour] = useState("/entreprises");
  useEffect(() => { setRetour(lienRetourListe("entreprises")); }, []);
  const [editInteraction, setEditInteraction] = useState<Interaction | null>(null);
  /** Message d'origine quand on est en train d'écrire une réponse. */
  const [repondreA, setRepondreA] = useState<Interaction | null>(null);
  /** Identifiants des interlocuteurs, par interaction. */
  const [interlocuteurs, setInterlocuteurs] = useState<Record<string, string[]>>({});
  const [notesDe, setNotesDe] = useState<Contact | null>(null);
  const [notesParContact, setNotesParContact] =
    useState<Record<string, { total: number; epinglee: boolean }>>({});
  ...
  const load = useCallback(async () => {
    if (!id) return;
    const [co, ct, it, ac, fu, tk, nt, vs, cn] = await Promise.all([
    ...
    ((cn.data as { contact_id: string; pinned: boolean }[]) ?? []).forEach((x) => {
      const e = compte[x.contact_id] ?? { total: 0, epinglee: false };
      compte[x.contact_id] = { total: e.total + 1, epinglee: e.epinglee || x.pinned };
    });
    setNotesParContact(compte);
    const echanges = (it.data as unknown as Interaction[]) ?? [];
    setInteractions(echanges);

    // Un message peut avoir plusieurs interlocuteurs : on lit la table de
    // liaison à part plutôt que de l'imbriquer dans la requête, pour garder
    // des requêtes simples et prévisibles.
    if (echanges.length) {
      const { data: liens } = await supabase.from("interaction_contacts")
        .select("interaction_id,contact_id")
        .in("interaction_id", echanges.map((e) => e.id));
      const parMessage: Record<string, string[]> = {};
      ((liens as { interaction_id: string; contact_id: string }[]) ?? []).forEach((l) => {
        parMessage[l.interaction_id] = [...(parMessage[l.interaction_id] ?? []), l.contact_id];
      });
      setInterlocuteurs(parMessage);
    } else setInterlocuteurs({});
  ...
  useEffect(() => { load(); }, [load]);
  useRealtime(["companies", "contacts", "interactions", "follow_ups", "tasks", "notes", "contact_notes", "visits", "activity_log"], load, `co-${id}`);

  /** Noms des interlocuteurs d'un message, avec repli sur l'ancien champ unique. */
  const nomsInterlocuteurs = (i: Interaction) => {
    const ids = interlocuteurs[i.id] ?? (i.contact_id ? [i.contact_id] : []);
    return ids
      .map((cid) => fullName(contacts.find((x) => x.id === cid)))
      .filter(Boolean)
      .join(", ");
  };
  ...
  return (
    <div className="space-y-5">
      <Link href={retour} className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-brand-600">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          {canWrite && (
            <div className="flex flex-wrap gap-2">
              ...
              <Button size="sm" onClick={() => { setEditInteraction(null); setRepondreA(null); setModal("interaction"); }}><MessageSquarePlus size={14} /> Interaction</Button>
        ...
        <Card>
          {contacts.length === 0 ? (
            ...
            <div className="divide-y divide-slate-100 dark:divide-slate-800">
              {contacts.map((ct) => (
                <div key={ct.id} className="flex flex-wrap items-center justify-between gap-3 p-4">
                  <div className="flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">
                        {ct.position ?? "—"}{ct.department ? ` · ${ct.department}` : ""}
                        {(ct.city || ct.country) && (
                  ...
                  {/* Les notes sont au centre de la ligne : c'est ce qu'on vient
                      lire avant d'écrire, avant même de regarder les étiquettes. */}
                  <div className="flex flex-1 justify-center">
                    <Button size="sm" variant="ghost" onClick={() => setNotesDe(ct)}
                      title="Notes et commentaires">
                      <MessageSquare size={14} />
                      {notesParContact[ct.id]?.total ? (
                        <span className="text-[11.5px]">{notesParContact[ct.id].total}</span>
                      ) : null}
                      {notesParContact[ct.id]?.epinglee && (
                        <span className="ml-0.5 rounded-sm bg-amber-100 px-1 text-[10px] font-semibold text-amber-800 dark:bg-amber-950 dark:text-amber-300">
                          !
                        </span>
                      )}
                    </Button>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Badge className={ct.relevance === 3 ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300" : ct.relevance === 2 ? "bg-slate-100 text-slate-600" : "bg-slate-100 text-slate-400 dark:bg-slate-800 dark:text-slate-400"}>
                      Pertinence {ct.relevance === 3 ? "élevée" : ct.relevance === 2 ? "moyenne" : "faible"}
                    </Badge>
                    {canWrite && (
                      <Button size="sm" variant="ghost" title="Écrire à ce contact"
                        onClick={() => setEcrireA(ct)}><Send size={13} /></Button>
                    )}
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          <Card className="p-5">
            ...
            <ol className="space-y-5">
              {enFils(interactions).map((fil) => (
                <li key={fil.id}>
                  <MessageInteraction i={fil} canWrite={canWrite} personnes={nomsInterlocuteurs(fil)}
                    onModifier={() => { setEditInteraction(fil); setRepondreA(null); setModal("interaction"); }}
                    onRepondre={() => { setEditInteraction(null); setRepondreA(fil); setModal("interaction"); }}
                    nbReponses={fil.reponses?.length ?? 0} />

                  {/* Les réponses se lisent sous leur message d'origine, dans
                      l'ordre où elles sont arrivées : c'est la suite logique
                      de l'échange, pas une nouvelle entrée de l'historique. */}
                  {!!fil.reponses?.length && (
                    <ol className="ml-4 mt-2 space-y-2 border-l-2 border-slate-200 pl-4 dark:border-slate-700">
                      {fil.reponses.map((r) => (
                        <li key={r.id}>
                          <MessageInteraction i={r} canWrite={canWrite} reponse personnes={nomsInterlocuteurs(r)}
                            onModifier={() => { setEditInteraction(r); setRepondreA(null); setModal("interaction"); }}
                            onRepondre={() => { setEditInteraction(null); setRepondreA(fil); setModal("interaction"); }} />
                        </li>
                      ))}
                    </ol>
          ...
          {!visit ? (
            ...
            <>
              <div className="mb-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <Badge className={visit.confirmation === "confirmee" ? "bg-green-100 text-green-800 dark:bg-green-950 dark:text-green-300"
                    : visit.confirmation === "annulee" ? "bg-red-100 text-red-700 dark:bg-red-950 dark:text-red-300" : "bg-amber-100 text-amber-800"}>
      ...
      <MessageModal open={Boolean(ecrireA)} onClose={() => setEcrireA(null)}
        contact={ecrireA}
        entreprise={{ id: c.id, name: c.name, city: c.city, sector: c.sector, last_interaction_at: c.last_interaction_at }} />
      ...
      <InteractionModal open={modal === "interaction"} companyId={c.id} contacts={contacts}
        interaction={editInteraction} parent={repondreA}
        onClose={() => { setModal(null); setEditInteraction(null); setRepondreA(null); }} onSaved={load} />
...
function MessageInteraction({
  i, canWrite, personnes, reponse, nbReponses = 0, onModifier, onRepondre,
}: {
  i: Interaction;
  canWrite: boolean;
  /** Interlocuteurs déjà mis en forme par la page. */
  personnes: string;
  /** Une réponse s'affiche en retrait, sans son propre bouton « Répondre ». */
  reponse?: boolean;
  nbReponses?: number;
  onModifier: () => void;
  onRepondre: () => void;
}) {
  const sens = directionMeta(i.direction);
  const entrant = i.direction === "recu";
  ...
  return (
    <div className={cx(
      ...
      <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
        <span className={cx("h-2 w-2 shrink-0 rounded-full", sens.puce)} />
        <span className="text-sm font-medium">{fmtDate(i.occurred_at)}</span>
        <span className="text-sm text-slate-500">{labelOf(CHANNELS, i.channel)}</span>
        <Badge className={sens.chip}>{sens.label}</Badge>
        {nbReponses > 0 && (
          <span className="text-[11px] text-slate-400">
            {nbReponses + 1} messages dans ce fil
          </span>
        )}
      </div>

      <p className="mt-0.5 text-xs text-slate-500">
        {/* Un message reçu vient de l'interlocuteur ; un message envoyé part
            vers lui. Écrire « → » dans les deux cas inverserait le sens. */}
        {entrant
          ? `${personnes || "Entreprise"} → ${fullName(i.author) || "nous"}`
          : `${fullName(i.author) || "—"}${personnes ? ` → ${personnes}` : ""}`}
        {" · "}{labelOf(OUTCOMES, i.outcome)}
      </p>

      {i.notes && (
        <p className="mt-1 rounded-lg bg-slate-50 p-2 text-xs text-slate-600 dark:bg-slate-800/60 dark:text-slate-300">
          {i.notes}
        </p>
      )}
      {i.message_sent && (
        <details className="mt-1">
          <summary className="cursor-pointer text-[11px] text-slate-500 hover:text-brand-700">
            {entrant ? "Message reçu" : "Message envoyé"}
          </summary>
      ...
      {canWrite && (
        <div className="mt-1 flex gap-3">
          <button onClick={onRepondre} className="text-[11px] font-medium text-brand-600 hover:text-brand-800">
            {reponse ? "Répondre au fil" : "Répondre"}
          </button>
          <button onClick={onModifier} className="text-[11px] text-slate-400 hover:text-brand-700">
            Modifier
          </button>
        </div>
      )}
