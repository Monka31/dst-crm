...
import {
  ...
  LogOut, PanelLeftClose, PanelLeftOpen, KeyRound,
} from "lucide-react";
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  const { session, profile, loading, signOut, canWrite, settings } = useApp();
  ...
  const loadAlerts = useCallback(async () => {
    ...
    const [fu, tk, vs, noContact] = await Promise.all([
      ...
      supabase.from("companies").select("id,name").eq("owner_id", profile.id)
        .in("status", ["a_identifier", "a_contacter"]).limit(5),
    ]);
  ...
  const nav = NAV.filter((n) => {
    if (n.href === "/statistiques" && !settings?.show_statistics) return false;
