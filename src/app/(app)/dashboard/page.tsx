...
import { EntonnoirSankey, type ParPoleStatut } from "@/components/EntonnoirSankey";
...
export default function DashboardPage() {
  ...
  /** Comptage pôle × statut, source de l'entonnoir en bas de page. */
  const [parPoleStatut, setParPoleStatut] = useState<ParPoleStatut>({});
  ...
  const load = useCallback(async () => {
    ...
    ((comps.data as { pole_id: string | null; status: string; sector: string | null }[]) ?? []).forEach((c) => {
      ...
      sec[nom] = (sec[nom] ?? 0) + 1;
      flux[k] = flux[k] ?? {};
      flux[k][c.status] = (flux[k][c.status] ?? 0) + 1;
    });
    setByPole(agg);
    setParPoleStatut(flux);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Entonnoir de prospection : ce que les compteurs ne montrent pas, à
          savoir où le portefeuille se perd entre l'identification et la
          visite confirmée. Placé en bas parce qu'il se lit à tête reposée,
          pas d'un coup d'œil en réunion. */}
      <Card>
        {/* Pas de compteur global dans l'en-tête : il contredirait le
            diagramme dès qu'un pôle est sélectionné. Les chiffres sont portés
            par les nœuds eux-mêmes, et suivent le filtre. */}
        <div className="border-b border-slate-200 px-5 py-3 dark:border-slate-800">
          <h2 className="card-title">Entonnoir de prospection</h2>
        </div>
        <EntonnoirSankey parPoleStatut={parPoleStatut} poles={poles} />
      </Card>
    </div>
  );
}
