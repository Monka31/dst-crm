...
export default function ResumePage() {
  const { canWrite, poles, settings, profile, editeurs } = useApp();
  ...
  const silencieux = editeurs.filter((m) => !actifs.has(m.id));
...
function detail(e: Evenement): string {
  switch (e.action) {
    ...
    case "interaction_received": return `Réponse reçue (${labelOf(CHANNELS, e.label)}) —`;
